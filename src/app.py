...
def npc(slug, npc_id):
    ...
    rel_data = db.compute_npc_relationship(npc_obj, known_events=viewer_known_events, is_dm=is_dm)
    ...
    return render_template("npc.html", meta=meta, npc=npc_obj, slug=slug,
                           ...
                           viewer_known_events=viewer_known_events,
                           current_session=db.get_current_session(slug),
...
def dm_edit_npc(slug, npc_id):
    db.update_npc(
        ...
        dm_notes=request.form.get("dm_notes", "").strip() or "",
    )
