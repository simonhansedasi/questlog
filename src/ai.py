...
- Party-directed events: when an NPC or faction acts UPON the party — gives, teaches, assists, attacks, deceives, instructs, rewards, threatens, heals, equips, warns, welcomes, sells to, or otherwise directs action at the party — log it ONLY as entity_type "party_group" with actor_id set to that NPC/faction id. Do NOT also create a separate NPC entry for the same event. Ask: "who received or was affected by this action?" If the answer is the party, the entry belongs on party_group, not on the NPC. WRONG: {entity_type:"npc", entity_name:"Bea Coldwell", note:"Bea welcomed the party and gave them rooms", actor_id:null}. RIGHT: {entity_type:"party_group", actor_id:"bea_coldwell", actor_type:"npc", note:"[[Bea Coldwell]] gave the party a warm welcome and set them up with rooms", polarity:"positive"}. Only log an event directly to an NPC when something happens TO that NPC — they are harmed, exposed, lose something, gain something for themselves, their plans change — not when they are the one doing something to the party.
- When in doubt whether to log to the NPC or to the party: if the party is the recipient or target of the action, always use party_group.
...
- Inter-entity events — generate TWO entries ONLY when the two perspectives are meaningfully different: when an NPC or faction acts upon another NPC/faction and the party did not cause it, generate a second entry on the acting entity ONLY IF the acting entity's polarity would differ from the affected entity's polarity, OR if the party's view of the actor changed in a way worth recording (non-null polarity). If both entries would share the same polarity (e.g., both negative in a pure combat exchange), generate ONLY ONE entry on the AFFECTED entity with actor_id set — the actor_id already captures who did it without a noisy mirror entry. EXCEPTION: when the acting entity is a FACTION and their activity is systematic or ongoing (extortion, occupation, control, oppression, blockade), ALWAYS generate a faction entry as well — a faction's pattern of behavior is distinct from a single combat event and must appear on the faction's own log.
  ...
  Example requiring BOTH a faction entry AND a victim entry — Rook Gang extorts Tomas Redd: entry on Tomas (actor_id=rook_gang, polarity="negative") + entry on the Rook Gang faction (polarity="negative" — from party's view, they are criminal aggressors). Do NOT skip the Rook Gang entry just because Tomas's entry already names them as actor.
- Reported faction activities: when NPCs describe an ongoing faction operation — extortion, shakedown, occupation, blockade, running people out of town — log it as a faction entry even if the party did not directly witness it. NPC testimony is sufficient to establish world state. Two or more NPCs independently describing the same faction activity is strong evidence — always log the faction entry. Leave actor_id null (the party is learning about it, not the actor); polarity reflects the party's view of the faction based on what they heard.
